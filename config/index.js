...
/**
 * Largest screenshot the image proxy will stream, in bytes.
 */
...
export function loadConfig(env = process.env) {
  ...
  const imageHosts = [origin(base), ...parseOrigins(env.IMAGE_HOSTS)].filter(Boolean);

  return {
    port: readPositiveIntOrThrow(env, "PORT", 3000),
    wpApiBase: base.replace(/\/+$/, ""),
    wpTimeoutMs: readPositiveIntOrThrow(env, "WP_TIMEOUT_MS", 5000),
    imageTimeoutMs: readPositiveIntOrThrow(
      env,
      "IMAGE_TIMEOUT_MS",
      readPositiveIntOrThrow(env, "WP_TIMEOUT_MS", 5000)
    ),
