...
describe("loadConfig", () => {
  ...
  it("rejects an absolute FALLBACK_IMAGE, which would leak a third-party origin", () => {
    expect(() =>
      loadConfig({ WP_API_BASE: "https://wp.example.com/x", FALLBACK_IMAGE: "https://cdn.test/a.png" })
    ).toThrow(/FALLBACK_IMAGE/);
  });

  it("defaults the image timeout to the WordPress timeout, and caps image size at 10 MB", () => {
    const cfg = loadConfig({ WP_API_BASE: "https://wp.example.com/x", WP_TIMEOUT_MS: "250" });
    expect(cfg.imageTimeoutMs).toBe(250);
    expect(cfg.maxImageBytes).toBe(10_485_760);
  });

  it("reads the image proxy overrides from the environment", () => {
    const cfg = loadConfig({
      WP_API_BASE: "https://wp.example.com/x",
      IMAGE_TIMEOUT_MS: "1500",
      MAX_IMAGE_BYTES: "2048",
    });
    expect(cfg.imageTimeoutMs).toBe(1500);
    expect(cfg.maxImageBytes).toBe(2048);
  });
});
