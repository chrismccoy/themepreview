...
describe("selection", () => {
  it("swaps the preview image, address, and label once the transition completes", () => {
    ...
    vi.runAllTimers();
    vi.useRealTimers();
    expect(document.getElementById("preview-image").getAttribute("src")).toBe("/media/theme/basalt");
  ...
  it("crossfades over exactly the duration the markup declares", () => {
    ...
    expect(image.getAttribute("src")).toBe("/media/theme/aurora");
    vi.advanceTimersByTime(1);
    expect(image.getAttribute("src")).toBe("/media/theme/basalt");
    vi.useRealTimers();
  });
